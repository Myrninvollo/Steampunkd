...
public class GuiHandbook extends GuiScreen {
	...
	void drawHeader() {
		...
		fontRendererObj.drawSplitString(StatCollector.translateToLocal("steampunked.gui.handbook.header"), left + 15, top + 20, 110, 0);
	...
	protected void actionPerformed(GuiButton par1GuiButton) {
		...
		HandbookCatagory category = i >= categoryList.size() ? null : categoryList.get(i);
	...
	void populateIndex() {
		...
		for(int i = 3; i < 12; i++) {
			...
			HandbookCatagory category = i_ >= categoryList.size() ? null : categoryList.get(i_);
			if(category != null)
